...
RAPTOR（Recursive Abstractive Processing for Tree-Organized Retrieval）由 Sarthi 等人于 ICLR 2024 提出，是一种旨在解决长上下文理解和全局推理挑战的检索增强生成（RAG）架构。

#### 2.3.1 核心理念
传统的 RAG 系统主要依赖于检索离散的、短片段的文本块，这种方式虽然在局部信息检索上表现良好，但往往难以捕捉文档的宏观结构和全局语义。RAPTOR 提出了一种递归的抽象处理机制，通过自底向上构建具有多级抽象的树状索引结构，使得模型不仅能够检索到具体的细节信息，还能获取高层的概括性知识。这种多粒度的检索能力显著增强了模型在处理长文档问答和综合性推理任务时的性能。

#### 2.3.2 树状索引构建 (Tree Construction)
RAPTOR 的索引构建过程是一个迭代的“聚类-摘要”循环，具体步骤如下：

1.  **文本切分与向量化 (Chunking & Embedding)**：
    首先，将长文档切分为固定或语义相关的短文本块（Chunks），并利用预训练的嵌入模型（如 SBERT）将其映射到高维向量空间。

2.  **降维与软聚类 (Dimensionality Reduction & Soft Clustering)**：
    -   **降维**：为了缓解高维空间中的稀疏性问题并优化聚类效果，采用 UMAP（Uniform Manifold Approximation and Projection）算法对嵌入向量进行降维处理。
    -   **软聚类**：在低维空间中，应用高斯混合模型（Gaussian Mixture Models, GMM）执行软聚类。与 K-Means 等硬聚类算法不同，GMM 允许一个文本块以一定的概率属于多个聚类，从而更有效地捕捉文本的多义性和跨主题特性。聚类的最优数量通过贝叶斯信息准则（BIC）自动确定。

3.  **递归摘要生成 (Recursive Summarization)**：
    对每个聚类内的文本块，利用大语言模型（如 GPT-4 或 GPT-3.5-turbo）生成综述性摘要。这些摘要不仅概括了聚类内的核心信息，还保留了关键的细节。

4.  **层次化构建 (Hierarchical Build)**：
    生成的摘要被视为上一层级的节点，重复上述的嵌入、聚类和摘要过程。该过程递归进行，直至聚类收敛或达到根节点，最终形成一棵包含从微观细节到宏观概括的树状索引结构。

#### 2.3.3 检索策略 (Retrieval Strategies)
针对不同类型的查询需求，RAPTOR 提供了两种主要的检索模式：

1.  **树遍历检索 (Tree Traversal Retrieval)**：
    该模式从树的根节点出发，逐层计算查询向量与节点的相似度，选择 Top-k 个最相关的节点，并递归地在其子节点中进行搜索。这种自顶向下的检索方式特别适用于需要深入特定分支的细节查询。

2.  **折叠树检索 (Collapsed Tree Retrieval)**：
    该模式将树结构扁平化，把所有层级的节点（包括原始文本块和各级摘要）合并为一个统一的候选集合，并在此集合上进行全局检索。实验结果表明，折叠树检索在大多数任务中表现更为优异，因为它能够同时召回高层的宏观摘要和底层的微观细节，从而在综合性问答任务中取得最佳平衡。

#### 2.3.4 方法优势
-   **全局语义捕捉**：通过递归生成的摘要，有效保留了文档的宏观主题和逻辑结构。
-   **多粒度信息融合**：支持在不同抽象层级上进行检索，灵活适应从细节查询到综述性问答的多种需求。
-   **长文档理解增强**：从根本上解决了传统 RAG 在处理书籍、法律文书等长文档时面临的“迷失中间” (Lost-in-the-Middle) 问题，提升了上下文的连贯性和完整性。
...
在完成基于高斯混合模型的节点聚类后，将每个聚类内的节点文本作为输入送入语言模型进行摘要生成，以获得该聚类的简洁、连贯的上层语义表示。该摘要步骤能够将检索到的潜在大规模内容压缩为可管理的语义单元，从而降低后续检索与生成的上下文负担，并为树结构的父节点提供高层概括。
