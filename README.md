...
#### 2.3.1 问题动机与方法概述
- 传统 RAG 面向长文档时，扁平片段检索难以同时覆盖全局主题与细节证据，导致上下文组织失衡与推理链条断裂。
- RAPTOR 以树状索引作为结构性约束，通过递归构建多级抽象节点，使系统在粗细粒度间实现可控检索与证据整合。
- 方法概述：在叶层以短片段承载细节，在中间层以聚类摘要承载中观主题，于顶层以全局摘要承载宏观结构；检索时可选择自顶向下的层次遍历或面向全层的折叠树检索。
 
#### 2.3.2 树状索引构建
- 文本分块与嵌入：将长文档切分为语义连续的短片段，并使用句子级嵌入模型编码为向量表示。
- 降维与软聚类：为提升聚类稳定性与区分度，先对嵌入进行降维（如 UMAP），再采用高斯混合模型（GMM）进行软聚类，以后验概率刻画样本的多隶属关系；聚类数通过信息准则（如 BIC）确定；参数估计采用 EM 算法。
- 递归摘要生成：对每个聚类的成员片段进行模型化摘要，形成父节点表示；再对父节点重复嵌入、聚类与摘要过程，直至形成覆盖从细节到全局的层次结构。
- 索引产物与组织：树状索引由叶节点（片段）、中间节点（聚类摘要）与根节点（全局摘要）构成，支持跨层级的内容寻址与证据汇聚。
 
#### 2.3.3 检索策略
- 层次遍历检索：自顶向下计算查询与各层节点的相关性，逐层选择最相关分支并下钻至叶层，适用于需要在特定主题脉络中精准定位细节的查询。
- 折叠树检索：将各层节点统一到同一候选集合中进行全局排序，同时召回宏观与微观证据，适用于综合型问答与跨主题关联性较强的任务。
 
#### 2.3.4 复杂度与工程考量
- 计算链条：嵌入计算、降维与聚类、摘要生成与递归构建构成离线索引流水线；查询时的层次遍历或折叠排序构成在线检索流程。
- 资源配置：离线阶段可采用批处理与缓存策略降低构建成本；在线阶段需在检索覆盖、延迟与上下文窗口预算之间做权衡。
- 稳定性与鲁棒：软聚类与信息准则避免过拟合与簇不稳定；在摘要阶段控制提示与长度以维持主题一致性与事实忠实度。
 
#### 2.3.5 评估框架
- 检索质量：以 Precision@k、Recall@k、MRR、NDCG 衡量跨层候选的相关性与排序质量。
- 生成质量：以 BLEU、ROUGE、BERTScore 与人工评审衡量回答的忠实度、完整性与可读性。
- 系统指标：以延迟、吞吐与构建时间衡量工程性能，并在不同层级召回比例下进行精度—效率曲线分析。
 
#### 2.3.6 讨论与局限
- 摘要误差传播：父节点摘要的偏差可能沿层级传播，需通过多源证据校验与再检索缓解。
- 主题边界模糊：跨主题文本的聚类边界受参数与降维影响，软聚类有助于降低边界硬判带来的信息丢失。
- 成本控制：树结构带来额外索引构建与维护成本，需结合应用场景确定层数与摘要力度。
 
#### 2.3.7 小结
- RAPTOR 以层次化组织克服扁平检索在长文档中的结构性局限，在多粒度上实现证据可寻址与上下文有序化；结合层次遍历与折叠树两类检索模式，能够在宏观综述与微观取证之间取得兼顾的覆盖与准确性，为后续的混合索引与多维相似性融合奠定结构基础。
...
为概括多维度索引的表示方式，可将节点的多属性表征记作 Φ(n)，并写为：
...
为保证工程可用性，索引构建引入缓存与持久化策略，对相同文档与配置哈希复用既有树结构以降低重复构建成本，并为在线阶段提供分层或折叠式候选集合入口。参数化配置覆盖 max_tokens、num_layers、reduction_dimension、threshold、summarization_length、cluster_embedding_model 与 selection_mode 等关键项，便于在精度与效率之间进行可控权衡。
