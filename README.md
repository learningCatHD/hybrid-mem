...
为了支持高效的查询时检索，系统维护一个基于向量嵌入的检索索引。每当新记忆被添加时，系统会将其内容与元数据组合成检索文档，更新到检索器的文档集合和嵌入矩阵中。检索器使用余弦相似度计算查询与记忆之间的语义相关性，支持按需返回 top-k 个最相关的记忆索引。

随着记忆数量的增长和演化操作的执行，已有记忆的元数据可能会发生变化（例如标签更新、上下文重写），为了确保检索索引与记忆状态保持一致，系统实现了定期整合机制（consolidate_memories）。当演化计数器达到预设阈值（默认每 100 次演化触发一次）时，系统会重新初始化检索器，将所有记忆的当前状态（包括更新后的上下文、标签和关键词）重新编码为嵌入向量，重建整个检索索引。这种设计在保证检索准确性的同时，避免了每次演化都重建索引带来的计算开销，在效果与效率之间取得了平衡。

**长期记忆层的整体架构**

从系统架构角度看，长期记忆层由三个核心组件协同工作：`MemoryNote` 类负责单个记忆单元的数据结构与元数据生成，`AgenticMemorySystem` 类负责记忆集合的管理、关联分析和演化决策，`SimpleEmbeddingRetriever` 类负责向量索引的构建与语义检索。这三个组件共同实现了"写入即索引、关联即演化、查询即检索"的完整流程，使得长期记忆层不仅是一个存储系统，更是一个能够自我组织和持续优化的知识网络。

这种设计的关键优势在于，它把记忆的组织工作从查询阶段前移到写入阶段，通过 LLM 的语义理解能力在记忆创建时就建立丰富的元数据和关联关系，从而为后续的查询时检索提供了高质量的知识基础。相比于传统方法在查询时才进行语义匹配，这种"写入时组织、查询时精读"的分工使得系统能够在保持检索效率的同时，显著提升复杂问题下的证据召回质量。
