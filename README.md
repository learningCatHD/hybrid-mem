...
Agent Memory 可以理解为智能体的“外部长期记忆”，它把历史对话、任务过程和关键结论保存下来，并在后续问题中按需调用。之所以需要这类机制，是因为模型一次能处理的上下文是有限的，无法长期稳定地保留跨会话、跨任务的重要信息。如果没有外部记忆，系统往往会重复查找同样的信息，或者在长链路任务中遗失关键背景。
 
因此，在本研究语境下，Agent Memory 的作用并不只是“多存一些内容”，而是让系统在长期运行中保持连续性、解释性与稳定性。连续性体现在历史经验能够被持续复用，解释性体现在答案能够追溯到来源证据，稳定性体现在面对复杂问题时能够有步骤地组织信息，而不是依赖一次偶然命中。

#### 3.1.2 长期记忆层：如何存储与组织经验

长期记忆层的核心任务是把分散的历史经验整理为可检索、可理解、可复用的知识基础。它并不是简单按时间堆叠日志，而是强调在写入阶段就保留问题背景、事件含义和结果信息，使后续检索能够快速定位“哪段经验与当前问题真正相关”。
 
随着经验累积，单条检索很容易出现信息碎片化，命中内容彼此割裂、难以支撑完整推理。为了解决这一问题，长期层需要把相关经验组织成可关联的结构，让系统在命中一条线索后可以顺着关联继续补齐上下文。这样一来，系统解决的不再只是“是否存得下”，而是“能否在复杂问题下把证据组织完整”。

#### 3.1.3 记忆演化机制

如果长期记忆只做“写入和检索”，随着时间推移通常会出现冗余堆积、表达不一致和噪声增加的问题。因此，记忆系统必须具备持续演化能力，即在引入新经验时同步检查并更新旧经验的组织方式，使相同主题的信息逐步收敛、冲突信息得到修正、过时信息被弱化。
 
从效果上看，这种演化机制让记忆从“静态档案”转变为“可自我整理的知识网络”。它的价值在于，新信息不仅被记录下来，还会反向改善旧信息的可用性。对长期任务而言，真正决定推理质量的往往不是某条记录是否存在，而是多条记录之间能否形成清晰、连贯、可更新的证据关系。

#### 3.1.4 查询侧记忆使用

完成长期记忆组织之后，系统还需要解决“如何在回答时真正用好记忆”这一问题。实践表明，复杂问题通常无法通过一次检索就拿到全部关键证据，更可行的方式是让系统按步骤完成“查找、阅读、判断、再补充”，并根据当前证据是否充分动态调整下一步。这样，记忆不再只是被动背景，而是参与推理过程的主动资源。
 
基于这一认识，后续 3.2 节将进一步展开查询阶段的自主取证机制，重点说明系统如何在可追踪、可预算、可中断的条件下逐步逼近答案，而不是依赖单次检索的偶然命中。
...
从整体上看，本工作提出的 test-time Agentic Memory 并不把记忆系统视为“存完即用”的静态仓库，而是把它升级为可在回答过程中被主动调度的推理资源。传统记忆增强方法往往在存储阶段投入较多设计，但在查询阶段仍以一次性召回为主，容易出现证据命中偶然、复杂问题支持不足等情况。针对这一不足，我们把系统能力从“被动调用记忆”推进到“主动组织证据”，使模型能够围绕当前问题逐步取证、持续校验并最终收敛到更可靠的答案。

这一创新首先体现在架构思想上。系统不再把记忆使用压缩为单步检索，而是区分“长期记忆沉淀”与“查询时决策取证”两类职责：前者负责稳定积累历史经验并保持语义连续性，后者负责在具体问题下动态选择证据路径并控制检索节奏。两者分工明确、协同工作，使系统既能保持长期记忆的稳定性，又能在推理阶段具备更强的针对性和适应性，从而避免“记得很多但用得不准”的常见问题。
...
在查询机制上，我们进一步把原本单一的向量检索操作改造为可组合、可解释的分步流程，使证据获取过程具备由浅入深的层次推进能力。模型先依据问题中的显式线索做初步筛选，再通过语义匹配缩小范围，随后对高相关记忆进行深入阅读并结合关联内容扩展局部证据，最后在证据充分时主动停止检索并进入回答生成。这种设计将“固定一次调用”转为“多步受控取证”，让检索过程从黑箱结果变成可观察、可复盘的推理轨迹，显著提升了复杂问题下的证据利用效率与答案稳定性。

为了确保这种主动检索不仅有效而且可控，系统在查询阶段引入了清晰的过程追踪与预算约束机制，使每一步“已获取了什么证据、还缺什么关键信息、还可投入多少检索成本”都可以被明确判断。这样一来，记忆使用过程不再是难以复盘的黑箱，而是可观察、可约束、可优化的决策过程。最终，这一范式把记忆增强从“提升召回概率”的局部改进，推进为“兼顾效果、成本与可解释性”的系统能力升级，为复杂任务中的稳定回答提供了更坚实的基础。

#### 3.2.2 构建长期记忆层

长期记忆层的构建是 test-time Agentic Memory 系统的基础，它负责将原始对话内容转化为结构化、可检索、可演化的知识单元。本工作基于 A-MEM 框架，采用 LLM 驱动的元数据生成与动态链接建立机制，使记忆系统不仅能够存储历史经验，还能在写入阶段就形成语义关联网络，为后续的查询时检索提供高质量的知识基础。

**长期记忆层的整体架构**

长期记忆层的核心目标并非单纯扩大历史信息的存储规模，而是将经验持续组织为可复用的知识结构。该层在长期运行过程中需要同时满足两项目标：其一，及时吸收新增信息，以保证后续任务能够利用最新证据；其二，持续整理既有信息，以避免记忆规模增长后出现语义割裂、线索冲突与检索偏移。

因此，本文将该架构设计为“写入即组织、运行中演化、周期性校准”的闭环机制。新增经验进入系统后，不再停留于原始记录层，而是经由语义化归纳后纳入既有知识关系；随着任务推进，系统持续评估经验之间的关联强度，以维持记忆网络的主题连贯性；在更长时间尺度上，通过一致性校准修复局部更新累积造成的语义偏移，从而保持整体结构稳定。

从整体架构视角看，该闭环可表述为“输入驱动、记忆组织、检索服务、反馈校准”四个环节的连续协同。输入环节负责将实时交互转化为可沉淀信息；记忆组织环节负责将新旧经验映射至统一语义空间并建立关联；检索服务环节面向查询任务提供可直接使用的证据；反馈校准环节依据回答质量与长期漂移风险对前述环节进行反向修正。上述环节并非一次性串行执行，而是持续循环推进，进而逐步提升知识结构质量。

该设计的另一关键在于将“局部响应最优”与“全局语义一致”分层处理。局部层面，系统优先保证新增经验能够快速进入检索流程，以满足实时性要求；全局层面，系统通过周期性校准抑制长期结构偏移，避免局部更新累积后破坏整体语义秩序。由此可见，该双节奏机制能够在响应速度与长期稳定性之间形成有效平衡，并提升系统可解释性与可维护性。

对于上层问答任务而言，证据来源不再局限于临时拼接的信息片段，而是来自持续整理后的知识网络。该机制能够提升系统在复杂问题中的跨时间、跨主题证据组织能力，并增强多轮推理过程中的上下文一致性。由此可见，长期记忆层实现了由“被动历史保存”向“主动决策支撑”的功能转变，为后续检索与生成提供更高质量的结构化基础。
...
该架构的关键价值在于将记忆系统由“被动存储”提升为“主动组织”，从而使后续查询不再依赖偶然命中，而能够在已组织的经验网络上实现更稳定的证据检索与推理。

**记忆单元的创建与元数据生成**

当新的对话内容需要写入记忆系统时，系统首先创建一个基本记忆条目。基于原子化记录原则，每个条目只承载一个独立、可复用的经验单元。在记忆集合中，我们将第 i 条记忆简化表示为：

```text
m_i = (text_i, time_i, meta_i, vec_i, links_i)
```

其中，`text_i` 是原始内容，`time_i` 是时间信息，`meta_i` 是由模型补充的语义信息（关键词、标签、摘要），`vec_i` 是用于相似度检索的向量表示，`links_i` 是与其语义相关的历史记忆集合。

为了在原始内容和时间信息之外补充语义上下文，系统会调用模型生成元数据。该过程可简写为：

```text
meta_i = LLM(text_i, time_i, prompt)
```

系统会把原始内容、时间信息和提示模板一并输入模型，并要求输出三类结果：关键词用于精准匹配，摘要用于快速理解上下文，标签用于主题级筛选。这样设计后，每条记忆在写入时就同时具备“可读、可筛、可检索”的多层语义表示，既保留原始信息，又便于后续多粒度检索。

在元数据生成过程中，系统使用结构化 JSON Schema 约束输出格式，确保关键词、摘要和标签可直接用于后续处理。如果生成失败或解析出错，系统会回退到默认值（空列表或通用分类），保证写入流程稳定。

为了支持高效检索与关联，系统再将文本内容与元数据合并编码为向量：

```text
vec_i = Encoder(text_i + meta_i)
```

其中 `Encoder` 为文本编码器（默认使用 SentenceTransformer `all-MiniLM-L6-v2`）。这种“语义元数据 + 向量表示”的组合让系统既保留可解释信息，又保持相似度检索效率。

此外，每个记忆单元还会自动生成唯一标识符（UUID）、访问计数等基础元数据，为后续的演化追踪和统计分析提供支持。这种基于 Zettelkasten 原则的设计使得记忆系统不仅能够存储历史经验，还能在写入阶段就形成语义关联网络，为后续的查询时检索提供高质量的知识基础。

**记忆关联网络的建立**

为避免依赖人工预设规则，系统采用“自主连边”机制：当新记忆 `m_n` 写入后，先基于向量语义在全量记忆中做相似度检索，再由 LLM 对候选集合进行关系判别并生成最终链接。该机制本质上把“高效筛选”与“语义理解”分离开来，前者保证可扩展性，后者保证连边质量。对于任意历史记忆 `m_j ∈ M`，其与新记忆的相似度定义为：

```text
(4) s_{n,j} = (e_n · e_j) / (||e_n|| ||e_j||)
```

系统随后按 `s_{n,j}` 排序，选出 top-k 候选邻居：

```text
(5) M_n^near = { m_j | rank(s_{n,j}) <= k, m_j ∈ M }
```

在候选集合确定后，系统把“新记忆 + 候选邻居 + 连边提示词”输入 LLM，生成链接集合：

```text
(6) L_n = LLM(m_n + M_n^near + P_s2)
```

链接输出可表示为：

```text
(7) L_n = { (m_n, m_a), (m_n, m_b), ..., (m_n, m_k) }
```

其中 `P_s2` 为关系判别提示模板，`L_n` 表示新记忆应连接的历史记忆集合。与仅依赖向量阈值的做法相比，这一流程能够识别更细粒度的语义关系，例如主题延续、因果依赖、证据互补与概念映射，从而在不进行全量两两比较的前提下构建更自然、更有组织性的记忆关联网络。

**检索索引的构建与维护**

检索索引采用“文本语义向量 + 文档映射表”的结构来支撑查询时召回。对每条记忆，系统会把原始内容与上下文、关键词、标签拼接为统一检索文本，再送入编码器生成向量表示。若记忆记为 `m_i`，其索引文档可写为：

```text
d_i = content_i + " , " + context_i + " " + join(keywords_i) + " " + join(tags_i)
```
...
查询阶段对问题 `q` 进行同源编码，并以余弦相似度排序返回 top-k 结果：

```text
score(q, i) = cos(Encoder(q), e_i)
```

```text
I_topk(q) = TopK(score(q, i), k)
```

在维护策略上，系统采用“写入时增量更新 + 演化后周期性重建”的双机制。新记忆写入后，会立即将其检索文本追加到索引语料并增量计算向量，保证新信息可被立刻检索；同时，记忆演化可能会改写已有条目的上下文和标签，导致旧向量与最新语义不一致，因此系统维护演化计数器，在达到阈值 `T`（默认 `T=100`）时触发一次全量重建：重新初始化检索器，并用当前全部记忆的最新状态重新编码。
...
这种维护方式避免了“每次微小演化都全量重建”的高成本，同时也防止索引长期漂移，能在检索时延与语义一致性之间保持较稳健的工程平衡。


#### 3.2.3 Test-Time Agentic Query

Test-Time Agentic Query 的目标是在不改变长期记忆存储形态的前提下，将查询阶段由“一次性检索”升级为“可规划、可迭代、可约束”的取证过程。其核心思想是把查询时行为显式建模为决策闭环：模型先根据当前问题与已有证据规划下一步动作，再调用对应检索操作获取新证据，随后更新状态并评估是否收敛。该机制与 A-MEM 的长期记忆组织能力形成互补关系，即长期层负责“知识沉淀与结构化组织”，查询层负责“任务导向的动态取证与证据整合”。

从架构角度看，Test-Time Agentic Query 由四个关键单元协同组成：行动规划器、检索工具集、查询状态跟踪器与答案生成器。规划器输出结构化动作，检索工具集提供不同粒度的证据访问接口，状态跟踪器维护查询上下文与预算消耗，答案生成器在收敛条件满足时生成最终响应。由此可见，该设计将传统黑箱检索过程拆解为可观测、可分析、可优化的显式流程。

**Test-Time Agentic Query 架构图**

```mermaid
flowchart LR
    Q[用户问题] --> P[行动规划器]
    P -->|keyword_search| K[词面检索]
    P -->|semantic_search| S[语义检索]
    P -->|read_memory| R[记忆读取与邻域扩展]
    P -->|final_answer| A[答案生成]
...
在执行逻辑上，系统首先完成查询初始化，包括关键词种子生成、已读记忆集合初始化、候选集合初始化与轨迹容器初始化。随后进入受预算约束的迭代循环：每一轮由规划器基于当前问题、已读记忆、候选状态、已聚合上下文及上一步工具反馈，输出一个 JSON 结构化动作。动作空间包含四类原子操作：词面检索、语义检索、记忆读取与显式收敛。该设计使“决策—执行”关系保持一一对应，能够有效避免隐式推理路径难以复盘的问题。

当规划器选择检索动作时，系统执行对应工具并回写状态。词面检索侧重高召回候选发现，语义检索侧重语义对齐与候选收敛，记忆读取侧重证据细化与局部邻域扩展。每次工具执行后，系统均更新候选集合、已读集合、聚合上下文、轨迹记录与检索开销估计。若规划器给出 `final_answer`，或达到最大循环次数，则终止迭代并进入答案生成阶段。若循环结束后仍未形成充分上下文，系统执行回退策略以保证输出可用性。

在回答生成阶段，系统将聚合证据映射为类别敏感的回答提示模板，针对时间推断、对抗式问答等任务施加更严格的输出约束。该流程在保持推理灵活性的同时，引入了明确的预算边界与收敛判据，从而实现效果与成本的可控权衡。

**执行逻辑伪代码**

```text
Algorithm 1: Test-Time Agentic Query
Input:
  q: user question
  M: long-term memory index
  L_max: maximum query-time loop budget
Output:
  a: final answer
  τ: action trajectory
  c: retrieval cost

1:  read_ids ← ∅, candidates ← ∅, context ← ""
2:  feedback ← ∅, τ ← [], c ← 0
3:  k_seed ← GenerateKeywords(q)
4:  for t = 1 to L_max do
5:      α_t ← PlanAction(q, k_seed, read_ids, candidates, context, feedback)
6:      if α_t.action = final_answer then
7:          τ ← τ ∪ {(t, α_t)}
8:          break
9:      end if
10:     if α_t.action = keyword_search then
11:         r_t ← KeywordSearch(M, α_t.keywords, α_t.top_k)
12:         candidates ← r_t.candidates
13:     else if α_t.action = semantic_search then
14:         r_t ← SemanticSearch(M, α_t.query, α_t.top_k)
15:         candidates ← r_t.candidates
16:     else if α_t.action = read_memory then
17:         r_t ← ReadMemory(M, α_t.memory_ids, read_ids)
18:         context ← Merge(context, r_t.observation)
19:         read_ids ← read_ids ∪ r_t.new_ids
20:     else
21:         r_t ← FallbackSearch(M, q)
22:         candidates ← r_t.candidates
23:     end if
24:     c ← c + EstimateTokens(r_t.observation)
25:     feedback ← BuildFeedback(α_t, r_t)
26:     τ ← τ ∪ {(t, α_t, Meta(r_t))}
27: end for
28: if context = "" then
29:     context ← FallbackContext(M, q)
30: end if
31: a ← GenerateAnswer(q, context, category)
32: return a, τ, c
```

Test-Time Agentic Query 的主要优势体现在以下方面。其一，该机制将检索过程显式化并全程记录轨迹，使查询行为具备可解释性与可审计性。其二，通过多轮取证与动态动作选择，系统可在复杂问题中逐步补全证据链，降低单次召回失配导致的回答脆弱性。其三，预算约束与收敛判据为计算开销提供了可控边界，支持在性能与成本之间进行量化权衡。其四，由于查询层升级不依赖长期存储格式重构，该方法具备较好的工程兼容性，能够在保留既有记忆资产的条件下实现增量式能力提升。研究表明，这种“长期记忆组织 + 查询时自主取证”的双层协同设计，更适合长对话与多跳推理场景中的稳定问答需求。



### 3.3 实验设计与分析

本节围绕长期记忆增强检索系统在长对话问答场景中的有效性展开实证分析。实验部分遵循“研究动机—数据与评价—实现细节—结果记录—结论归纳”的组织方式，旨在系统刻画方法性能、行为特征及其适用边界。

#### 3.3.1 实验设计动机

本文的实验设计围绕三项核心问题展开。其一，在长上下文与多类型问答条件下，检索增强记忆机制能否稳定提升回答质量。其二，不同问题类别（多跳、时间、开放域、单跳、对抗）是否呈现结构化性能差异。其三，查询阶段的自主取证过程是否具备可解释性、可观测性与可优化性。上述问题共同构成了方法有效性与工程可用性的双重验证目标。

基于上述目标，实验评估不仅报告终态指标，还同步记录检索路径、动作序列与检索开销，以形成“结果指标评估 + 过程行为评估”的统一分析框架。由此可见，实验结论既关注“回答得是否更好”，也关注“如何达到该结果”。

#### 3.3.2 数据集特点与任务划分

实验数据采用 LoCoMo 子集（`locomo10`）。该子集包含 10 个对话样本、272 个会话片段、5882 条对话轮次与 1986 个问答对，具备长会话、多时间线与多主题交织等特征，能够有效检验模型在跨轮信息整合与长期一致性维护方面的能力。

按照任务类型，问答划分为五类：多跳推理（Category 1）、时间推断（Category 2）、开放域问答（Category 3）、单跳事实（Category 4）与对抗式问答（Category 5）。对应样本规模分别为 282、321、96、841、446。研究表明，该分布具有显著类别不均衡特征，其中单跳与对抗式样本占比相对较高。

在采样策略上，评测脚本在 `ratio < 1.0` 时采用顺序截取前 `N` 个样本，而非随机抽样。对于当前 10 样本数据集，`ratio=0.01` 与 `ratio=0.1` 均对应 `N=1`，因此两种设置均形成 199 题评测集，其类别分布为 32/37/13/70/47（对应 Category 1-5）。

#### 3.3.3 实验配置与执行流程

实验执行采用自动化评测流程，核心配置包括：检索候选规模 `retrieve_k=10`、查询阶段最大检索循环次数为 3、对抗类别温度参数 `temperature_c5=0.5`。在执行路径上，系统先构建并缓存对话记忆，再逐题完成“检索—读取—回答”流程，同时记录回答结果与轨迹行为。

评价指标覆盖词级匹配与语义相似两类维度，包括 Exact Match、F1、ROUGE-1/2/L、BLEU、BERT-F1、METEOR 与 SBERT 相似度。为与主文分析重点保持一致，结果部分主要报告 ROUGE-2 与 ROUGE-L（百分制），并结合轨迹统计分析检索策略的收敛行为与成本特征。实验输出分为两类：其一为结果文件（包含 overall、分类型与逐题指标）；其二为轨迹文件（包含动作序列与检索开销）。由此可见，该设计能够同时支持性能比较与机制解释。

#### 3.3.4 完整实验记录

`reference/A-mem/results/summary_table.md` 与对应结果文件共记录 7 次有效运行。完整记录如下（ROUGE 为百分制）：

| 运行文件 | 模型 | Ratio | 问题数 | Overall R2 | Overall RL | Multi-hop R2/RL | Temporal R2/RL | Open-domain R2/RL | Single-hop R2/RL | Adversarial R2/RL |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| gpt-4o-mini_locomo10_ratio0.01_2026-02-14-08-06.json | gpt-4o-mini | 0.01 | 199 | 20.75 | 28.51 | 1.77/10.51 | 17.49/28.50 | 1.74/10.88 | 20.03/29.16 | 42.55/44.68 |
| gpt-4o-mini_locomo10_ratio0.01_2026-02-14-17-06.json | gpt-4o-mini | 0.01 | 199 | 17.88 | 26.71 | 2.77/12.55 | 14.94/30.87 | 2.51/12.94 | 19.76/28.96 | 31.91/33.54 |
| gpt-4o-mini_locomo10_ratio0.01_2026-02-14-21-29.json | gpt-4o-mini | 0.01 | 199 | 20.12 | 29.32 | 4.46/11.59 | 20.88/33.15 | 3.66/13.77 | 17.72/29.07 | 38.30/43.06 |
| gpt-4o-mini_locomo10_ratio0.1_2026-02-14-22-56.json | gpt-4o-mini | 0.1 | 199 | 20.35 | 28.49 | 2.18/10.72 | 23.41/38.74 | 0.00/8.34 | 21.64/30.86 | 34.04/34.55 |
| gpt-4o-mini_locomo10_ratio0.1_2026-02-15-10-25.json | gpt-4o-mini | 0.1 | 199 | 20.40 | 28.95 | 4.07/9.95 | 27.93/42.10 | 0.00/7.51 | 21.36/31.95 | 29.79/32.99 |
| gpt-4o-mini_locomo10_ratio0.1_2026-02-15-11-56.json | gpt-4o-mini | 0.1 | 199 | 18.91 | 27.19 | 1.39/9.13 | 28.01/40.63 | 0.00/2.78 | 18.32/28.41 | 29.79/33.84 |
| qwen3-max_locomo10_ratio0.1_2026-02-15-14-59.json | qwen3-max | 0.1 | 199 | 16.04 | 26.37 | 5.59/13.75 | 23.02/38.15 | 4.10/18.34 | 22.98/36.57 | 10.64/12.74 |

#### 3.3.5 轨迹记录统计

`trace` 目录包含 5 份轨迹日志。统计如下：

| 轨迹文件 | 问题数 | final_answer 闭环率(%) | 平均检索开销 | P90 开销 | 最大开销 | 动作分布（keyword/semantic/read/final） |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| gpt-4o-mini_locomo10_ratio0.01_2026-02-14-21-29_trajectory.json | 199 | 0.00 | 904.64 | 1032 | 1400 | 580 / 12 / 0 / 0（另有 fallback 5） |
| gpt-4o-mini_locomo10_ratio0.1_2026-02-14-22-56_trajectory.json | 199 | 19.60 | 1932.14 | 2322 | 2598 | 507 / 3 / 413 / 39 |
| gpt-4o-mini_locomo10_ratio0.1_2026-02-15-10-25_trajectory.json | 199 | 75.88 | 922.21 | 1177 | 1484 | 259 / 0 / 176 / 151 |
| gpt-4o-mini_locomo10_ratio0.1_2026-02-15-11-56_trajectory.json | 199 | 42.21 | 1091.11 | 1484 | 1866 | 255 / 72 / 179 / 84 |
| qwen3-max_locomo10_ratio0.1_2026-02-15-14-59_trajectory.json | 199 | 48.74 | 1603.83 | 2451 | 3250 | 290 / 10 / 197 / 97 |

#### 3.3.6 结果分析与结论

第一，实验记录显示该评测框架具有较强的过程可观测性，但模型表现存在显著波动。以 `gpt-4o-mini` 为例，`ratio=0.1` 的三次运行 Overall ROUGE-2 分别为 20.35、20.40、18.91，表明在相同数据规模下仍存在可观方差。由此可见，实验结论应基于多次运行统计，而不宜由单次结果直接外推。

第二，类别差异显著且稳定存在。`gpt-4o-mini` 在 Temporal 与 Adversarial 上总体更强，而 `qwen3-max` 在 Multi-hop、Open-domain 与 Single-hop 上更优。该现象说明，test-time Agentic Memory 提供的是统一的取证机制，并不抹平基础模型在时间表达、对抗鲁棒性与开放域概念对齐上的能力差异。

第三，轨迹统计表明“闭环率”与“检索开销”之间并非线性关系。闭环率较高的运行（如 75.88%）并不必然带来更高开销，反而可能因为更早收敛而降低平均检索成本；闭环率较低或策略偏保守的运行，容易出现多轮检索而未及时终止，导致 `retrieved_tokens` 上升。由此可见，收敛策略是影响成本效率的关键变量。

第四，`ratio<1` 时采用顺序截取而非随机采样，这一设定虽然提高了复现实验的一致性，但会降低统计外推性，特别是在类别不均衡数据上更易放大样本偏置。后续研究宜补充随机多次抽样或完整数据评测，并报告均值与方差，以提升结论稳健性。

综合而言，实验结果支持以下结论：该系统在“可观测检索过程”与“可控取证开销”方面具有明确工程价值；但其最终问答质量仍受到基础模型能力、收敛策略与采样协议共同影响。由此可见，后续优化应同时覆盖策略层（收敛判据与预算分配）与评测层（多次随机试验与方差报告）。
